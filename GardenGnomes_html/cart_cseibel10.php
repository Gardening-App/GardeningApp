...
<html>
    <head>
        ...
        <link href="minisMall.css" rel="stylesheet">
    </head>
    <body>
        ...
        foreach ($_POST as $productID => $productQty) {
            ...
            echo $productID;
            if (is_int($productQty) && $productQty > 0 && !$remove[$row[$productID]]) {
                $cart['productID'] = $productQty;
            } elseif (!$productQty) { //|| $remove[$row[$productID]]) {
		...
		foreach($cart as $item => $itemQty) {
            $prodIDStr += $item . ",";
            echo $itemQty . " of item adding\n";
            //$_SESSION['numItems'] += $itemQty;
        }
