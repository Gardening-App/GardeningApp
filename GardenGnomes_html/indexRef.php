...
<body>
    ...
    // in a log file.  Remove thse from production code.
    ...
    function callQuery($pdo, $query, $error) {
        ...
        return $pdo->query($query);
            ...
            // Note that we should get rid of all system-generate error
    ...
    // Check to see fi the user clicked the Add a new book title link
    ...
    if (!empty(trim($newBookTitle)) && $newBookTitle != "Enter book title") {
        ...
        // Replace any single quotes in out book title with 
        // an escapted single quote so our query doesn't fail.
        ...
        if (!$numBookTitles) { // new book wasn't found, so add it

            echo("\t<h3 style=\"color: #fff;\">New book title $newBookTitle added</h3>\n");

            // Now that we know that we want to add a new book,
            ...
            if (!$numAuthorRows) {
                ...
                try {
                    ...
                    // automatically.
                    //
                    // Use a transaction as a fail-sage to roll back the database
                    // to it's previous state if something goes wrong with the query.
                    //
                    ...
                    $s = $pdo->prepare($sql);   // $s is a PDOStatement object because that is what the pepare() method of the PDO class returns
                ...
                echo("\t<h3 style=\"color: #fff;\">New book title $newBookAuthor already exists - not added</h3>\n");
            }
            ...
            // Get the new book's category id
            ...
            // We are finally ready to insert the new book!!!
            try {
